...
export class InitialSeeder {
  ...
  private async createDentistUser() {
    const dentistRole = await this.roleRepository.findOneBy({
      nombre: Roles.DENTIST,
    });
    if (!dentistRole)
      throw new NotFoundException('Rol de Dentista no encontrado');
    const usuarios = [
      {
    ...
    for (const usuario of usuarios) {
      const existeCorreo = await this.userRepository.findOneBy({
  ...
  private async createAdministrativeUser() {
    const administrativeRole = await this.roleRepository.findOneBy({
      nombre: Roles.ADMINISTRATIVE,
    });
    if (!administrativeRole)
      throw new NotFoundException('No se encontro el rol Administrativo');
  ...
  private async createHorarios() {
    ...
    for (const horario of horariosData) {
      ...
      if (!existe) {
        ...
        console.log(
          `Horario ${horario.horaInicio} - ${horario.horaFin} creado`,
        );
      }
    }
  }
  ...
  private async createHorariosFecha() {
    ...
    const dentistas = await this.dentistRepository.find({
      relations: ['usuario'],
    });
      ...
      for (const dentista of dentistas) {
        for (const horario of horarios) {
          ...
          if (!existe) {
            ...
            console.log(
              `HorarioFecha creado: ${fecha.toISOString().split('T')[0]} - ${horario.horaInicio}-${horario.horaFin} con ${dentista.usuario.nombre}`,
            );
          }
        }
      }
  ...
  private async createPacientes() {
    ...
    if (estadosCiviles.length === 0) {
      console.log(
        'No hay estados civiles. Primero ejecuta el seeder de EstadoCivil.',
      );
      return;
    }
    ...
    for (const data of pacientes) {
      const edad =
        new Date().getFullYear() - data.fecha_nacimiento.getFullYear();

      const estadoCivil =
        estadosCiviles[Math.floor(Math.random() * estadosCiviles.length)];
  ...
  private async createFamiliares() {
    ...
    for (const paciente of pacientes) {
      ...
      for (let i = 0; i < numFamiliares; i++) {
        const nombre =
          nombresEjemplo[Math.floor(Math.random() * nombresEjemplo.length)];
        const parentesco =
          parentescos[Math.floor(Math.random() * parentescos.length)];
        ...
        if (!existe) {
          ...
          console.log(
            `Familiar agregado: ${nombre} (${parentesco}) -> Paciente: ${paciente.nombre}`,
          );
        }
      }
    }
  ...
  private async createCitas() {
    ...
    for (const hf of horariosFecha) {
      ...
      if (!existe) {
        ...
        console.log(
          `Cita creada: Paciente ${paciente.nombre} - ${hf.horario.horaInicio}-${hf.horario.horaFin} con ${hf.dentista.usuario.nombre}`,
        );
      }
